...
let
  ...
  mkRustMultiDrv = { rev, url, sha256, cargoHash ? null, cargoLock ? null
    , cargoBuildFlags ? [ ], binsFilter ? null, binaryPackagesResult ?
    ...
    let
      ...
      cargoHashOrLock = if cargoHash != null then {
        cargoHash = builtins.trace cargoHash cargoHash;
      } else if cargoLock != null then {
        cargoLock = {
          lockFile = builtins.trace "${src}/Cargo.lock" "${src}/Cargo.lock";
          outputHashes =
            builtins.trace cargoLock.outputHashes cargoLock.outputHashes;
        };
      } else
        throw
        "${url}@${rev} has not specified either of cargoHash or cargoLock";
